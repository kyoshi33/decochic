...
function Profil() {
  ...
  const clickMesLikes = () => {
    ...
    fetch(`https://decochicbackend.vercel.app/users/likesProducts?email=${email}&token=${token}`, {
      method: 'GET',
  ...
  const clickMesAchats = () => {
    const { email, token } = user;
    fetch(`https://decochicbackend.vercel.app/users/commandesProducts?email=${email}&token=${token}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
    })
      .then(response => response.json())
